<template>
  <div class="wrap">
    ...
    <div class="main">
      <div>
        <span style="display:block">可用余额：</span>
        {{balance}} ETW &asymp; {{eth}}ETH
      </div>
      <div class="form">
        <div>
          <div>提币币种：</div>
...
export default {
  ...
  watch: {
    coin_type(v) {
      if (this.coin_type == "ETW") {
        ...
        this.min =
          "请输入提币数量，至少" +
          parseFloat(parseFloat(this.min_num) / parseFloat(this.ratio)).toFixed(2) +
          this.coin_type;
      }
    }
  },
  methods: {
    ...
    num(e) {
      ...
      if (this.coin_type == "ETW") {
        if (parseFloat(this.u_num) >= parseFloat(this.min_num)) {
          this.u_number = parseFloat(this.u_num * (1 - this.fee / 100)).toFixed(
            4
          );
        } else {
          this.u_number = 0;
        ...
        if (parseFloat(this.u_num) >= parseFloat(this.min_num / this.ratio)) {
          this.u_number = parseFloat(this.u_num * (1 - this.fee / 100)).toFixed(
            4
          );
        } else {
      ...
      that
        ...
        .then(function(res) {
          if (res.data.code == 1) {
            ...
            that.ratio = res.data.data.balance/res.data.data.eth;
            that.min =
              "请输入提币数量，至少" + res.data.data.min + that.coin_type;
            ...
            if (res.data.data.address.etw.length) {
              ...
              for (
                let i = 0, l = res.data.data.address.etw.length;
                i < l;
                i++
              ) {
            ...
            if (res.data.data.address.eth.length) {
              that.u_ads = res.data.data.address.eth[0].id;
              for (
                let i = 0, l = res.data.data.address.eth.length;
                i < l;
                i++
              ) {
    ...
    sub() {
      let that = this;
      if (that.u_num && that.u_pwd && that.u_code) {
